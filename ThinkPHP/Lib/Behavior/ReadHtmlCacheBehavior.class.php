...
class ReadHtmlCacheBehavior extends Behavior {
    ...
    static private function requireHtmlCache() {
        // 分析当前的静态规则
         ...
         if(!empty($htmls)) {
            $htmls = array_change_key_case($htmls);//将数组所有键名改为[小写]
            // 静态规则文件定义格式 actionName=>array('静态规则','缓存时间','附加规则')
            // 'read'=>array('{id},{name}',60,'md5') 必须保证静态规则的唯一性 和 可判断性
            // 检测静态规则
            $moduleName = strtolower(MODULE_NAME);//取得小写Module名
            $actionName = strtolower(ACTION_NAME);//取得小写Action名
            ...
            if(!empty($html)) {
                ...
                if(!empty($html[2])) $rule    =   $html[2]($rule); // 规则数组第三个成员 执行附加函数
                $cacheTime = isset($html[1])?$html[1]:C('HTML_CACHE_TIME'); // 规则数组第二个成员 缓存有效期
                // 当前缓存文件
                define('HTML_FILE_NAME',HTML_PATH . $rule.C('HTML_FILE_SUFFIX'));//定义缓存文件名称至HTML_FILE_NAME常量,在WriteHtmlCacheBehavior行为中获取并写入缓存
                return $cacheTime;
            }
        }
        // 静态规则为空或规则为空,无需缓存
